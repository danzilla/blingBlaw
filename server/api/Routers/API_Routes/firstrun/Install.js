...
// Bling_statements
const { is_app_Database, create_app_Database, create_app_Schema } = require('../../SQL_Statement/firstRun_sql_statement');
const { create_Table_UserAuth, create_Table_UserDetails } = require('../../SQL_Statement/user_sql_statement');
const { create_Table_fannyPackz } = require('../../SQL_Statement/fannyPack_sql_statement');
// Response
const RESPONSE = {
    Title: "Database Intialize",
    status: null,
    message: null,
    data: null
}
...
const Install = function (req, res, next) {
    ...
    async function Fire() {
        try {
            ...
            User_Response.message = `Database been initialised!`;
        } catch (error) {
            if (error.code == "42P04") {
