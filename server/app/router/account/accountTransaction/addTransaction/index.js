...
const { add_newTransaction_to_accountTransaction_table } = require('../../../../config/statement/account_sql_statement');
// addTransaction
const addTransaction = function (req, res, next) {
	...
	// Get FannyPack name
	if(!req.body.accountSerial || !req.body.accountTransaction || !req.body.fannyPack) {
		// pageMessage
	...
	} else if (req.body.accountTransaction && req.body.accountSerial && req.body.fannyPack) { 
		// If alll good
		// Collect Results
		const addAccountTransactionResult = [];
		// Payload bzz
		const payLoad = {
			transactionData: req.body.accountTransaction,
